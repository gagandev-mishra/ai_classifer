...
export default function Resnet() {
  ...
  const classify = async (uri: string) => {
    ...
    try {
      const res = await fetch('http://192.168.1.37:8000/classify/', {
        method: 'POST',
        body: formData,
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Stack.Screen options={{ title: "ResNet", headerShown: false }} />
      <HeaderText title="ResNet" />
      ...
      {results.length > 0 && results.map((r, i) => (
        <Text key={i} style={styles.resultText}>
          {r.label}: {(r.probability * 100).toFixed(2)}%
        </Text>
      ))}
    </ScrollView>
  );
